...
class LinkedInAdapterConnector(Connector):
    ...
    def get_oauth_authorize_url(self, state: str, code_challenge: str | None = None) -> str:
        raise NotImplementedError('LinkedIn adapter placeholder. Current implementation lives in app/connectors/linkedin.py.')

    async def exchange_code_for_token(self, code: str, code_verifier: str | None = None) -> TokenResult:
        raise NotImplementedError('LinkedIn adapter placeholder. TODO: map provider response to SMCC contract.')
    ...
    async def publish_text_link(self, account: OAuthAccount, payload: dict) -> str:
        raise NotImplementedError('LinkedIn adapter placeholder. TODO: implement adapter publish logic.')
